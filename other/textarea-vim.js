...
function moveWord(where, repeats, vim, isWORD, toEnd) {
    ...
    if (vim.mode === MODE_NORMAL && previousSelectionStart === where.selectionStart) {
        if (repeats > 0) {
    ...
    if (vim.mode === MODE_VISUAL && previousSelectionPos === getSelectionPosition(where, vim)) {
        if (repeats > 0) {
...
const COMMAND_RE =
    /^([1-9]\d*)?((dd|yy|>>|<<|[~\$\^A-EGI-JO-PSV-Wa-fhi-lo-pr-y]|gg|<C-r>)|(^0))(([1-9]\d*)?(gg|[ia][(){}[\]<>Ww]|[tf].|[\$\^0D-EGWehj-lw])|.)?/;
